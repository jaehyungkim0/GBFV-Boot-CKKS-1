...
func (ecd Encoder) EncodePoly(values interface{}, pt *rlwe.Plaintext) (err error) {
		switch values := values.(type) {
		case []float64:
			...
			Float64ToFixedPointCRT(ecd.parameters.RingQ().AtLevel(pt.Level()), values, 1.0, pt.Value.Coeffs)
		...
		case []*big.Float:
			...
			one := new(big.Float).SetPrec(ecd.Prec())
			one = new(big.Float).SetInt(big.NewInt(1))
			BigFloatToFixedPointCRT(ecd.parameters.RingQ().AtLevel(pt.Level()), values, one, pt.Value.Coeffs)
